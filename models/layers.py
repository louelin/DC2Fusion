...
class Conv3d(nn.Conv3d):
    ...
    def __init__(self, in_channels, out_channels, kernel_size, stride=1,
        ...
        if kernel_size == 1:
            ...
            ws = kernel_size
            self.avg_pool = nn.AdaptiveAvgPool3d((ws, ws, ws))

            self.num_lat = int((kernel_size * kernel_size * kernel_size) / 2 + 1)

            self.ce = nn.Linear(ws * ws * ws, self.num_lat, False)
            self.ce_bn = nn.BatchNorm1d(in_channels)
            self.ci_bn2 = nn.BatchNorm1d(in_channels)
            ...
            self.gd = nn.Linear(self.num_lat, kernel_size * kernel_size * kernel_size, False)
            self.gd2 = nn.Linear(self.num_lat, kernel_size * kernel_size * kernel_size, False)

            self.unfold = unfoldNd.UnfoldNd(kernel_size, dilation, padding, stride)
    ...
    def forward(self, x):
        ...
        if self.ind:
            ...
            if self.g > 3:
                oc = self.ci(
                    self.act(self.ci_bn2(ce2).view(b, c // self.g, self.g, -1).transpose(2, 3))).transpose(2,
                                                                                                              3).contiguous()
            else:
                oc = self.ci(self.act(self.ci_bn2(ce2).transpose(2, 1))).transpose(2, 1).contiguous()
            oc = oc.view(b, self.oc, -1)
            ...
            out = self.sig(out.view(b, 1, c, self.ks, self.ks, self.ks) + oc.view(b, self.oc, 1, self.ks, self.ks,
                                                                                  self.ks))
            x_un = self.unfold(x)  
            b, _, l = x_un.size()

            out = (out * weight.unsqueeze(0)).view(b, self.oc, -1)
            return torch.matmul(out, x_un).view(b, self.oc, d, h, w)  # 修改输出维度以适应3D数据
